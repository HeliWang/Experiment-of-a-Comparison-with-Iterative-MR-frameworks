...
//	static String global_init_inputPath;
	...
	public static class KMeansMapper extends MapReduceBase implements
		...
		public void init_centers()
		{
			
			reducerOutput
					.put(
							"1",
							parseStringToVector("10110,2540"));
			reducerOutput
					.put(
							"2",
							parseStringToVector("33310,471"));
			reducerOutput
					.put(
							"3",
							parseStringToVector("61190,97100"));
			reducerOutput
					.put(
							"4",
							parseStringToVector("51120,18640"));
			reducerOutput
					.put(
							"5",
							parseStringToVector("81,277"));
			reducerOutput
					.put(
							"6",
							parseStringToVector("9102,20391"));
			reducerOutput
					.put(
							"7",
							parseStringToVector("10192,8172"));
			reducerOutput
					.put(
							"8",
							parseStringToVector("2018,37112"));
			reducerOutput
					.put(
							"9",
							parseStringToVector("3060,7711"));
			reducerOutput
					.put(
							"10",
							parseStringToVector("40110,5"));		
		}
...
/*
	private static double[] parseStringToVector(String line, double[] row) {
		try {
			...
			{
				row = new double[size];		
				System.err.println("Row Init");
			}
			int i = 0;
			while (tokenizer.hasMoreTokens()) {
				String attribute = tokenizer.nextToken();
				System.err.println("size "+size+ " / i : "+i+" / attribute : " + attribute+"-");
				row[i] = Double.parseDouble(attribute);				
				i++;
			}
...
*/	
	...
	/*
	private static void accumulate(double[] array) {
		double sum[] = null;
		for (int i = 0; i < sum.length; i++)
		{
			sum[i] += array[i];
...
*/
			...
			while (values.hasNext()) {
				...
				accumulate(sum, row);
				//System.err.println("1. Id: " + key.toString() + " Value: " + sum[0] + " / "+ sum[1]+" / "+count);
				count++;
			}
